...
void Cadastro_Cript(){
    ...
    FILE *arquivo_Cripto  = fopen("criptomoedas.txt", "a");
...
void consultar_saldo_adm(int linha_usuario){
    ...
    float moedas[quantidade_criptomoedas];  // alocar diretamente na variável
    fscanf(fp, "%lld", &cpf);   
    fscanf(fp, "%d", &senha);   
    fscanf(fp, "%s", nome_investidor);   
    for (int k = 0; k < quantidade_criptomoedas + 1; k++)
        fscanf(fp, "%f", &moedas[k]);

    char sigla_cripto[quantidade_criptomoedas][4];
    char nome_cripto[quantidade_criptomoedas][20];
    float cotacao[quantidade_criptomoedas];
    float taxavenda[quantidade_criptomoedas];
    float taxacompra[quantidade_criptomoedas];
